public class SecurePassword {
    ...
    */
    public boolean isSecure() {
        ...
        return containsDigit() && containsLowercase() && containsUppercase() && containsSpecialSymbol() && isLongEnough();
    }
    ...
    */
    public String status() {
        String str = "";
            ...
            if (!containsDigit()) {
                str += "The password must contain a digit  \n";
            }
            if (!containsLowercase()) {
                str += "The password must contain a lowercase character  \n";
            }
            if (!containsUppercase()) {
                str += "The password must contain a uppercase character  \n";
            }
    ...
    private boolean checkString(String characterString) {
        for (int i = 0; i < characterString.length(); i++) {
            for (int k = 0;k < password.length(); k++) {
                if (password.substring(k, k+1).equals(characterString.substring(i, i+1))) {
                    return true;
                }
            }
        }
